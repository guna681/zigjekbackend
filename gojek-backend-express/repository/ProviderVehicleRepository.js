module.exports = function () {
  ...
  this.fetchActiveVehicle = (providerId) => {
    var output = {}
    return new Promise(function (resolve) {
      var knex = new Knex(config)
      knex(providerVehicle)
        .select('VehicleImage as vehicleImage', 'VehicleBrandName as brandName', 'VehicleModelName as modelName', 'VehicleNumber as noPlate', 'RideVehicleTypeId as serviceIds')
        .where('ProviderId', providerId)
        .where('Status', 'active')
        .then((result) => {
          if (result.length > 0) {
