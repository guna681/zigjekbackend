module.exports = function () {
  ...
  this.userRating = async (rating, callback) => {
    var response = {}
    try {
      var userRating = await ratingRepository.insertUserReview(rating)
      if (userRating.error) {
        response.error = true
  ...
  this.getUserAverageRating = (userId) => {
    ...
    return new Promise(async function (resolve) {
      try {
        var rating = await ratingRepository.fetchUserAverageRating(userId)
        if (rating.error) {
          response.error = true
  ...
  this.getProviderAverageRating = async (providerId) => {
    ...
    return new Promise(async function (resolve) {
      try {
        var rating = await ratingRepository.fetchProviderAverageRating(providerId)
        if (rating.error) {
          response.error = true
          response.msg = 'NO_DATA'
        } else {
  ...
  this.fetchRatingDetailsService = (rating) => {
    return new Promise(async function (resolve) {
      var response = {}
      try {
        var userRating = await ratingRepository.fetchRatingDetails(rating)
        if (userRating.error) {
          response.error = true
